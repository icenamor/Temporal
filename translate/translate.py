...
def pddl_to_sas(task, pddlFile):
    ...
    ##ISA antes instantiate.explore(task)
...
#        return unsolvable_sas_task("Simplified to trivially false goal")
    #ISA
   ## print "ISA should insert something here"
   ## print len(mutex_key)
    #3print len(strips_to_sas),"\nranges", len(ranges),"\ninit", len(task.init),"\ngoals", len(goal_list),"\nactions", len(actions), "\ndurative actions ",len(durative_actions), "\naxioms ",len(axioms),"\nnum_axions ", len(num_axioms)
    ##print "\nnum_axioms_by_layer", len(num_axioms_by_layer), "\nmax", max_num_layer,"\nnum_axiommap", len(num_axiom_map)
    #3print "\nconst_num_axioms", len(const_num_axioms)
    
    ##print "reachable ", len(reachable_action_params)
    ##print len(groups)
    ##print len(mutex_groups)
    ##print len(translation_key)
    ##print "end Isa new --- \n"
    
    pddlFile.mutex_key =  len(mutex_key)
    pddlFile.strips_to_sas = len(strips_to_sas)
    pddlFile.ranges = len(ranges)
    pddlFile.goal_list = len(goal_list)
    pddlFile.task_init = len(task.init)
    pddlFile.act = len(actions)
    pddlFile.durative_Act = len(durative_actions)
    pddlFile.axi = len(axioms)
    pddlFile.num_axioms =  len(num_axioms)
    pddlFile.num_axioms_by_layer = len(num_axioms_by_layer)
    pddlFile.max_num_layer = max_num_layer 
    pddlFile.num_axiom_map =  len(num_axiom_map)
    pddlFile.const_num_axioms =len(const_num_axioms)
    pddlFile.reachable = len(reachable_action_params)
    pddlFile.groups =  len(groups)
    pddlFile.mutex_group = len(mutex_groups)
    pddlFile.translation_key = len(translation_key)
    #ISA
...
if __name__ == "__main__":
    ...
    #ISA
    pddlFile = PddlFile(task.domain_name, task.task_name, len(task.requirements.requirements), len(task.types), len(task.objects), len(task.predicates), (len(task.init)- len(task.objects)), len(task.goal.parts), len(task.actions), len(task.axioms))
    ##pddlFile.passMinCost(task.use_min_cost_metric)
    #ISA
    pddlFile.durative_actions = len(task.durative_actions)
    pddlFile.axiom_counter = task.axiom_counter
    pddlFile.function_symbols = len(task.function_symbols)
    pddlFile.function_administrator = len(task.function_administrator.functions)
    ##print "\t\t", len(task.function_symbols),
