...
vector<simplyDTG> DomainTransitionGraphSymb::generate_cpp_input(ofstream &outfile, Store store, int variable) const{
  ...
  for(int i = 0; i < vertices.size(); i++) {
    ...
    for(int j = 0; j < vertices[i].size(); j++) {
      ...
      //cout<<trans.target <<" op "<<  trans.op<<endl;
      //duration:
      string  type_here = "-";
    ...
    //ISA
    simplyDTG simpltytg = simplyDTG(variable, vertices[i].size(), i, trans.target, number+1,trans.duration.var->get_level(), trans.duration.op, type_here);
   // simpltytg.print();
